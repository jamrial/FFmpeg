...
/** hash context */
typedef struct AVTiger {
    uint64_t count;       ///< number of bytes in buffer
    uint8_t  buffer[64];  ///< 512-bit buffer of input values used in hash updating
    uint64_t state[3];    ///< current hash value
} AVTiger;

const int av_tiger_size = sizeof(AVTiger);

struct AVTiger *av_tiger_alloc(void)
{
    return av_mallocz(sizeof(struct AVTiger));
}

static const uint64_t C[1024] = {
...
#define round(a,b,c,d)                                 \
    c ^= d;                                            \
    a -= C1[ c        & 0xFF] ^ C2[(c >> 16) & 0xFF] ^ \
         C3[(c >> 32) & 0xFF] ^ C4[(c >> 48) & 0xFF];  \
    b += C4[(c >>  8) & 0xFF] ^ C3[(c >> 24) & 0xFF] ^ \
         C2[(c >> 40) & 0xFF] ^ C1[(c >> 56) & 0xFF]

#define ROUND_0_TO_8(a,b,c,d)                          \
    round(a,b,c,d);                                    \
    b = (b << 2) + b

#define ROUND_9_TO_16(a,b,c,d)                         \
    round(a,b,c,d);                                    \
    b = (b << 3) - b

#define ROUND_17_TO_24(a,b,c,d)                        \
    round(a,b,c,d);                                    \
    b = (b << 3) + b
...
static void tiger_transform(uint64_t *state, const uint8_t buffer[64])
{
    uint64_t a, b, c;
    uint64_t block[8];
    int n;
    ...
    c = state[2];
    for (n = 0; n < 8; n++)
        block[n] = AV_RL64(buffer + 8 * n);

    ROUND_0_TO_8(a,b,c,block[0]);
    ROUND_0_TO_8(b,c,a,block[1]);
    ROUND_0_TO_8(c,a,b,block[2]);
    ROUND_0_TO_8(a,b,c,block[3]);
    ROUND_0_TO_8(b,c,a,block[4]);
    ROUND_0_TO_8(c,a,b,block[5]);
    ROUND_0_TO_8(a,b,c,block[6]);
    ROUND_0_TO_8(b,c,a,block[7]);
    MIX_KEY
    ROUND_9_TO_16(c,a,b,block[0]);
    ROUND_9_TO_16(a,b,c,block[1]);
    ROUND_9_TO_16(b,c,a,block[2]);
    ROUND_9_TO_16(c,a,b,block[3]);
    ROUND_9_TO_16(a,b,c,block[4]);
    ROUND_9_TO_16(b,c,a,block[5]);
    ROUND_9_TO_16(c,a,b,block[6]);
    ROUND_9_TO_16(a,b,c,block[7]);
    MIX_KEY
    ROUND_17_TO_24(b,c,a,block[0]);
    ROUND_17_TO_24(c,a,b,block[1]);
    ROUND_17_TO_24(a,b,c,block[2]);
    ROUND_17_TO_24(b,c,a,block[3]);
    ROUND_17_TO_24(c,a,b,block[4]);
    ROUND_17_TO_24(a,b,c,block[5]);
    ROUND_17_TO_24(b,c,a,block[6]);
    ROUND_17_TO_24(c,a,b,block[7]);
...
void av_tiger_init(AVTiger *ctx)
{
...
void av_tiger_update(AVTiger* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
    ...
    for (i = 0; i < len; i++) {
        ctx->buffer[j++] = data[i];
        if (64 == j) {
            tiger_transform(ctx->state, ctx->buffer);
            j = 0;
        }
    }
#else
    if ((j + len) > 63) {
        ...
        tiger_transform(ctx->state, ctx->buffer);
        for (; i + 63 < len; i += 64)
            tiger_transform(ctx->state, &data[i]);
        j = 0;
    } else
        i = 0;
...
void av_tiger_final(AVTiger* ctx, uint8_t *digest)
{
    int i;
    uint64_t finalcount = av_le2ne64(ctx->count << 3);

    av_tiger_update(ctx, "\1", 1);
    while ((ctx->count & 63) != 56)
        av_tiger_update(ctx, "", 1);
    av_tiger_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
    for (i = 0; i < 3; i++)
        AV_WL64(digest + i*8, ctx->state[i]);
}
...
void av_tiger_sum(uint8_t *dst, const uint8_t *src, const int len)
{
    ...
    av_tiger_init(&ctx);
    av_tiger_update(&ctx, src, len);
    av_tiger_final(&ctx, dst);
}
...
int main(void)
{
    ...
    unsigned char digest[64];
        ...
        for (k = 0; k < 3; k++) {
            av_tiger_init(&ctx);
            if (k == 0)
                av_tiger_update(&ctx, "abc", 3);
            else if (k == 1)
                av_tiger_update(&ctx, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56);
            else
                for (i = 0; i < 1000*1000; i++)
                    av_tiger_update(&ctx, "a", 1);
            av_tiger_final(&ctx, digest);
            for (i = 0; i < 24; i++)
                printf("%02X", digest[i]);
            putchar('\n');
        }
        //test vectors taken from http://www.cs.technion.ac.il/~biham/Reports/Tiger/
        printf("2aab1484e8c158f2 bfb8c5ff41b57a52 5129131c957b5f93\n"
               "0f7bf9a19b9c58f2 b7610df7e84f0ac3 a71c631e7b53f78e\n"
               "6db0e2729cbead93 d715c6a7d36302e9 b3cee0d2bc314b41\n");

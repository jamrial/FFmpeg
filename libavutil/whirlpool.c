...
/** hash context */
typedef struct AVWhirlpool {
    uint64_t count;       ///< number of bytes in buffer
    uint8_t  buffer[64];  ///< 512-bit buffer of input values used in hash updating
    uint64_t state[8];    ///< current hash value
} AVWhirlpool;

const int av_whirlpool_size = sizeof(AVWhirlpool);

struct AVWhirlpool *av_whirlpool_alloc(void)
{
    return av_mallocz(sizeof(struct AVWhirlpool));
}

static const uint64_t C0[10] = {
...
#if CONFIG_SMALL
#define C1(x) c1[x]
#define C2(x) ror(c1[x], 8)
#define C3(x) ror(c1[x], 16)
#define C4(x) ror(c1[x], 24)
#define C5(x) ror(c1[x], 32)
#define C6(x) ror(c1[x], 40)
#define C7(x) ror(c1[x], 48)
#define C8(x) ror(c1[x], 56)
#else
#define C1(x) c1[x]
#define C2(x) c2[x]
#define C3(x) c3[x]
...
static void whirlpool_transform(uint64_t *state, const uint8_t buffer[64])
{
    uint64_t I[8], J[8], K[8], L[8];
    int i, j;

    for (i = 0; i < 8; i++) {
		J[i] = AV_RB64(buffer + 8 * i) ^ (K[i] = state[i]);
    }

    for (i = 0; i < 5; i++) {
                               TRANSFORM(L, K, 0) ^ C0[i*2];
       for (j = 1; j < 8; j++) TRANSFORM(L, K, j);
       for (j = 0; j < 8; j++) TRANSFORM(I, J, j) ^ L[j];

                               TRANSFORM(K, L, 0) ^ C0[i*2+1];
       for (j = 1; j < 8; j++) TRANSFORM(K, L, j);
       for (j = 0; j < 8; j++) TRANSFORM(J, I, j) ^ K[j];
    }

    for (i = 0; i < 8; i++)
        state[i] ^= J[i] ^ AV_RB64(buffer + 8 * i);
}

void av_whirlpool_init(AVWhirlpool *ctx)
{
...
void av_whirlpool_update(AVWhirlpool* ctx, const uint8_t* data, unsigned int len)
{
    unsigned int i, j;
    ...
    for (i = 0; i < len; i++) {
        ctx->buffer[j++] = data[i];
        if (64 == j) {
            whirlpool_transform(ctx->state, ctx->buffer);
            j = 0;
        }
    }
#else
    if ((j + len) > 63) {
        ...
        whirlpool_transform(ctx->state, ctx->buffer);
        for (; i + 63 < len; i += 64)
            whirlpool_transform(ctx->state, &data[i]);
        j = 0;
    } else
        i = 0;
...
void av_whirlpool_final(AVWhirlpool* ctx, uint8_t *digest)
{
    int i;
    uint64_t finalcount = av_be2ne64(ctx->count << 3);

    av_whirlpool_update(ctx, "\200", 1);
    if ((ctx->count & 63) > 32)
        while (ctx->count & 63)
            av_whirlpool_update(ctx, "", 1);
    while ((ctx->count & 63) != 56)
        av_whirlpool_update(ctx, "", 1);
    av_whirlpool_update(ctx, (uint8_t *)&finalcount, 8); /* Should cause a transform() */
    for (i = 0; i < 8; i++)
...
void av_whirlpool_sum(uint8_t *dst, const uint8_t *src, const int len)
{
    ...
    av_whirlpool_init(&ctx);
    av_whirlpool_update(&ctx, src, len);
    av_whirlpool_final(&ctx, dst);
}
...
int main(void)
{
    ...
    unsigned char digest[64];
        ...
        for (k = 0; k < 4; k++) {
            av_whirlpool_init(&ctx);
            if (k == 0)
                av_whirlpool_update(&ctx, "abc", 3);
            else if (k == 1)
                av_whirlpool_update(&ctx, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789", 62);
            else if (k == 2)
                av_whirlpool_update(&ctx, "The quick brown fox jumps over the lazy dog", 43);
            else
                for (i = 0; i < 1000*1000; i++)
                    av_whirlpool_update(&ctx, "a", 1);
            av_whirlpool_final(&ctx, digest);
            for (i = 0; i < 64; i++)
                printf("%02X", digest[i]);
            putchar('\n');
        }
//test vectors
        printf("4e2448a4c6f486bb 16b6562c73b4020b f3043e3a731bce72 1ae1b303d97e6d4c "

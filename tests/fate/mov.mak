...
FATE_MOV_SAMPLES_FFMPEG_FFPROBE-$(call ALLYES, FILE_PROTOCOL OGG_DEMUXER            \
...
FATE_MOV_SAMPLES_FFMPEG_FFPROBE-$(call ALLYES, FILE_PROTOCOL MOV_DEMUXER MJPEG_DECODER \
...
FATE_MOV_SAMPLES_FFMPEG_FFPROBE-$(call ALLYES, FILE_PROTOCOL SRT_DEMUXER MOV_DEMUXER SUBRIP_DECODER TTML_ENCODER TTML_MUXER MOV_MUXER) += fate-mov-mp4-ttml-stpp fate-mov-mp4-ttml-dfxp
...
FATE_MOV_SAMPLES_FFMPEG_FFPROBE-$(call ALLYES, FILE_PROTOCOL PIPE_PROTOCOL \
...
FATE_SAMPLES_FFMPEG_FFPROBE += $(FATE_MOV_SAMPLES_FFMPEG_FFPROBE-yes)

tests/data/one_stream_per_channel.mov: TAG = GEN
tests/data/one_stream_per_channel.mov: tests/data/asynth-44100-1.wav tests/data/filtergraphs/mov-channel-description
tests/data/one_stream_per_channel.mov: ffmpeg$(PROGSSUF)$(EXESUF) | tests/data
	$(M)$(TARGET_EXEC) $(TARGET_PATH)/$< -nostdin \
        -i $(TARGET_PATH)/tests/data/asynth-44100-1.wav \
        -filter_complex_script $(TARGET_PATH)/tests/data/filtergraphs/mov-channel-description \
        -map "[outFL]" -map "[outFR]" -map "[outFC]" -map "[outLFE]" -map "[outBL]" -map "[outBR]" -map "[outDL]" -map "[outDR]" -c:a pcm_s16le \
        -fflags +bitexact -flags +bitexact \
        -y $(TARGET_PATH)/tests/data/one_stream_per_channel.mov 2>/dev/null

FATE_MOV_FFMPEG_FFPROBE-$(call ALLYES, FILE_PROTOCOL PIPE_PROTOCOL \
                               WAV_DEMUXER PAN_FILTER \
                               PCM_S16LE_DECODER PCM_S16LE_ENCODER \
                               MOV_MUXER MOV_DEMUXER) \
                          += fate-mov-channel-description
fate-mov-channel-description: tests/data/one_stream_per_channel.mov
fate-mov-channel-description: CMD = run ffprobe$(PROGSSUF)$(EXESUF) -show_entries stream=channel_layout $(TARGET_PATH)/tests/data/one_stream_per_channel.mov

FATE_FFMPEG_FFPROBE += $(FATE_MOV_FFMPEG_FFPROBE-yes)

fate-mov: $(FATE_MOV) $(FATE_MOV_FFPROBE) $(FATE_MOV_FASTSTART) $(FATE_MOV_FFMPEG_FFPROBE-yes) $(FATE_MOV_SAMPLES_FFMPEG_FFPROBE-yes)

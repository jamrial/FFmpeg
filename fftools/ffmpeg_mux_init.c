    ...
    while (token) {
        ...
        while (subtoken) {
            ...
            if (element) {
                ...
                element = submix->elements[submix->num_elements - 1];
                element->audio_element = oc->stream_groups[idx];

                element->element_mix_config =
                    avformat_iamf_param_definition_alloc(AV_IAMF_PARAMETER_DEFINITION_MIX_GAIN, NULL, 0, NULL, NULL);
                if (!element->element_mix_config)
                    ret = AVERROR(ENOMEM);
                av_opt_set_dict2(element, &dict, AV_OPT_SEARCH_CHILDREN);
                element->element_mix_config->parameter_rate = stg->streams[0]->codecpar->sample_rate;
            } else if (layout) {

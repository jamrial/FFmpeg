...
static int libopus_check_vorbis_layout(AVCodecContext *avctx, int mapping_family) {
    av_assert2(avctx->ch_layout.nb_channels < FF_ARRAY_ELEMS(ff_vorbis_channel_layouts));
    ...
    if (avctx->ch_layout.order == AV_CHANNEL_ORDER_UNSPEC) {
        ...
        AVChannelLayout chl;
        av_channel_layout_from_mask(&chl, ff_vorbis_channel_layouts[avctx->ch_layout.nb_channels - 1]);
